...
#define RESET1 7    // Сброс запрета работы АВП. МОжно слделать удержанием в течение 10с
...
void setup()
{
  ...
  start1.setTimeout(5000); // Время для удержания в мс
}
...
void indicateError()
{
  if (digitalRead(UKK1) == true) // Сработало тепловое реле
  {
    flagError = true;
  }
  if (flagError == 1)
